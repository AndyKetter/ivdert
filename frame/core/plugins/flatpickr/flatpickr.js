...
function Flatpickr(element, config) {
	...
	function buildTime() {
		...
		if (!self.config.time_24hr) {
			...
			self.amPM = createElement("span", "flatpickr-am-pm", ["上午", "下午"][self.hourElement.value > 11 | 0]);
		...
		if (self.config.enableTime && !self.isMobile) {
			...
			if (readTimeInput) {
				...
				if (!self.config.time_24hr)
					...
					hours = hours % 12 + 12 * (self.amPM.innerHTML === "下午");
	...
	function timeWrapper(e) {
		...
		if (e.target.className === "flatpickr-am-pm") {
			e.target.textContent = ["上午", "下午"][e.target.textContent === "上午" | 0];
			e.stopPropagation();
			return;
		}
